...
本项目将全书的模式整合到一个单一的、生产级别的系统中。你将构建一个**类型安全的并发任务调度器**，它使用泛型、traits、typestate、通道、错误处理和测试。

**预计时间**：4-6 小时 | **难度**：★★★

> **你将练习：**
> - 泛型和 trait 边界（第 1-2 章）
> - 任务生命周期的 typestate 模式（第 3 章）
> - 零成本状态标记的 PhantomData（第 4 章）
> - worker 通信的通道（第 5 章）
> - 使用作用域线程的并发（第 6 章）
...
1. **任务**具有类型化的生命周期：`Pending → Running → Completed`（或 `Failed`）
2. **Workers** 从通道获取任务，执行它们，并报告结果
...
**你的工作**：实现 `WorkItem::new()` 接受任务名称和闭包。
添加 `TaskId` 生成器（简单的原子计数器或 mutex 保护的计数器）。
...
- `Scheduler::shutdown(self) -> Vec<TaskResult<R>>` —— drop sender，加入 workers，收集结果
...
1. **Happy path**：提交 10 个任务，关闭，验证所有 10 个结果都是 `Ok`
2. **错误处理**：提交失败的任务，验证 `TaskResult.outcome` 是 `Err`
3. **空调度器**：创建并立即关闭 —— 无 panic
4. **属性测试**（bonus）：使用 `proptest` 验证对于任意 N 个任务（1..100），调度器总是返回恰好 N 个结果
...
fn main() {
    ...
    for i in 0..20 {
        ...
        // 注意：.unwrap() 用于简洁 —— 在生产环境中处理 SendError。
        scheduler.submit(item).unwrap();
    }
...
|-----------|--------|
| 类型安全 | 无效状态转换无法编译 |
| 并发 | Workers 并行运行，无数据竞争 |
| 错误处理 | 所有失败都捕获在 `TaskResult` 中，无 panic |
| 测试 | 至少 3 个测试；bonus 给 proptest |
...
1. **优先级队列**：添加 `Priority` newtype（1-10）并优先处理更高优先级的任务
2. **重试策略**：失败的任务重试最多 N 次，然后标记为永久失败
3. **取消**：添加 `cancel(TaskId)` 方法移除待处理任务
4. **Async 版本**：使用 `tokio::sync::mpsc` 通道移植到 `tokio::spawn`（第 15 章）
5. **指标**：跟踪每个 worker 的任务计数、平均执行时间和失败率
