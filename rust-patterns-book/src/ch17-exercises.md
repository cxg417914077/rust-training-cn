...
使用 type-state 模式构建一个交通灯状态机。灯必须按 `Red → Green → Yellow → Red` 转换，其他顺序都不应该可能。
...
**关键要点**：无效的转换是编译错误，而非运行时 panic。

</details>

---

### 练习 2：带 PhantomData 的测量单位 ★★（约 30 分钟）
...
fn main() {
    ...
    // let bad = width + time; // ❌ 编译错误：不能相加 Meters + Seconds
}
```
...
### 练习 3：基于通道的 worker 池 ★★★（约 45 分钟）

使用通道构建一个 worker 池，其中：
- 调度器通过通道发送 `Job` 结构体
- N 个 worker 消费作业并发送结果回来
- 使用 `crossbeam-channel`（如果不可用则使用 `std::sync::mpsc`）
...
fn worker_pool(jobs: Vec<Job>, num_workers: usize) -> Vec<JobResult> {
    ...
    // 在 Arc<Mutex> 中包装接收器以便在 worker 之间共享
    ...
    drop(result_tx); // 丢弃我们的副本以便结果通道在 worker 完成时关闭
    ...
    drop(job_tx); // 关闭作业通道 —— worker 在 draining 后退出
...
**Bonus**：在阶段之间改变类型的通用管道将使用不同的设计 —— 每个 `.pipe()` 返回一个具有不同输出类型的 `Pipeline`（这需要更高级的泛型处理）。

</details>

---

### 练习 5：带 thiserror 的错误层次 ★★（约 30 分钟）
...
设计一个带有 `Error` 和 `Id` 关联类型的 `Repository<T>` trait。为内存存储实现它并演示编译时类型安全。
...
### 练习 7：Unsafe 之上的安全包装器（第 11 章）★★★（约 45 分钟）
...
**Hint**：使用 `MaybeUninit<T>` 和 `[const { MaybeUninit::uninit() }; N]`。
...
impl<T, const N: usize> Drop for FixedVec<T, N> {
    fn drop(&mut self) {
        // SAFETY: data[0..len] 都已初始化 —— drop 每一个。
        for i in 0..self.len {
...
- 支持尾随逗号
- 支持空调用 `map!{}`
- 适用于任何为 `Into<K>` 和 `Into<V>` 实现类型的类型以获得最大灵活性
...
设计一个 `Duration` 包装器，使用自定义 serde 反序列化器从人类可读的字符串（如 `"30s"`、`"5m"`、`"2h"`）反序列化。结构体还应该序列化回相同格式。
...
编写一个 async 函数 `fetch_all`，生成三个 `tokio::spawn` 任务，每个模拟使用 `tokio::time::sleep` 的网络调用。使用 `tokio::try_join!` 连接所有三个，并用 `tokio::time::timeout(Duration::from_secs(5), ...)` 包装。返回 `Result<Vec<String>, ...>` 或如果任何任务失败或截止日期过期则返回错误。
...
每个生成的任务返回 `Result<String, _>`。`try_join!` 解包所有三个。
将整个 `try_join!` 包装在 `timeout()` 中 —— `Elapsed` 错误意味着你达到了截止日期。
...
2. **Transformer**：从通道 A 读取，平方每个值，发送到通道 B。
3. **Consumer**：从通道 B 读取，收集到 `Vec<u64>`，返回它。

所有三个阶段作为并发 `tokio::spawn` 任务运行。使用有界通道演示背压。断言最终 vec 等于 `[1, 4, 9, ..., 400]`。

**学习目标**：`mpsc::channel`、有界背压、带 move 闭包的 `tokio::spawn`、通过通道关闭实现优雅关闭。
