...
> **你将学到：**
> - 模块布局约定和重新导出策略
> - 精制 crate 的公共 API 设计清单
> - 人体工学的参数模式：`impl Into`、`AsRef`、`Cow`
> - 使用 `TryFrom` 和已验证类型的"解析，不要验证"
> - 特性标志、条件编译和工作区组织
...
1. **接受引用，返回 owned** —— `fn process(input: &str) -> String`
2. **对参数使用 `impl Trait`** —— `fn read(r: impl Read)` 而不是 `fn read<R: Read>(r: R)`，签名更清晰
3. **返回 `Result`，而不是 `panic!`** —— 让调用者决定如何处理错误
4. **实现标准 trait** —— `Debug`、`Display`、`Clone`、`Default`、`From`/`Into`
5. **使无效状态不可表示** —— 使用类型状态和新类型
6. **对复杂配置使用 builder 模式** —— 如果字段是必需的则使用类型状态
7. **密封你不想让用户实现的 trait** —— `pub trait Sealed: private::Sealed {}`
8. **标记类型和函数为 `#[must_use]`** —— 防止静默丢弃重要的 `Result`、守卫或值。适用于任何忽略返回值几乎肯定是 bug 的类型：
   ```rust
   #[must_use = "丢弃守卫会立即释放锁"]
...
> **`#[non_exhaustive]`** —— 标记公共枚举和结构体，这样添加变体或字段不是破坏性变更。下游 crate 必须在 match 语句中使用通配符臂（`_ =>`），并且不能使用结构体字面量语法构造类型：
> ```rust
> #[non_exhaustive]
> pub enum DiagError {
>     Timeout,
>     HardwareFault,
>     // 在未来的版本中添加新变体不是语义化版本破坏。
> }
> ```

### 人体工学参数模式 —— `impl Into`、`AsRef`、`Cow`

最有影响力的 Rust API 模式之一是在函数参数中接受**最通用的类型**，这样调用者不需要在每个调用点重复 `.to_string()`、`&*s` 或 `.as_ref()`。这是 Rust 特定版本的"接受时宽容"。

#### `impl Into<T>` —— 接受任何可转换的内容
...
connect("localhost".to_string(), 5432);  // 讨厌的 .to_string()
connect(hostname.clone(), 5432);          // 如果已经有 String 则不必要的 clone

// ✅ 人体工学：接受任何可以转换为 String 的内容
fn connect(host: impl Into<String>, port: u16) -> Connection {
    let host = host.into();  // 在函数内部转换一次
    // ...
}
connect("localhost", 5432);     // &str —— 零摩擦
connect(hostname, 5432);        // String —— move，无 clone
```

这有效是因为 Rust 的 `From`/`Into` trait 对提供通用转换。
当你接受 `impl Into<T>` 时，你是在说："给我任何知道如何变成 `T` 的东西。"

#### `AsRef<T>` —— 借用为引用

`AsRef<T>` 是 `Into<T>` 的借用对应物。当你只需要*读取*数据而不需要所有权时使用它：
...
file_exists(Path::new("/tmp/test.txt"));  // 笨拙
...
`Cow<'a, T>`（Clone on Write）延迟分配直到需要修改。
它持有借用的 `&T` 或 owned 的 `T::Owned`。当大多数调用不需要修改数据时这很完美：
...
| `impl Into<String>` | Owned | 转换时 | 接受 &str、String —— 将存储/拥有 |
| `Cow<'_, str>` | 任一 | 仅在修改时 | 通常不修改的处理 |
| `&[u8]` / `impl AsRef<[u8]>` | 借用 | 从不 | 字节导向的 API |

> **`Borrow<T>` vs `AsRef<T>`**：两者都提供 `&T`，但 `Borrow<T>` 额外保证
> `Eq`、`Ord` 和 `Hash` 在原始形式和借用形式之间**一致**。
> 这就是为什么 `HashMap<String, V>::get()` 接受 `&Q where String: Borrow<Q>` —— 而不是 `AsRef`。
> 当借用形式用作查找键时使用 `Borrow`；对一般"给我引用"参数使用 `AsRef`。
...
impl DiagRunner {
    /// 对 name 接受任何字符串类型，对 config 接受任何路径类型。
    pub fn new(
...
一个将字符串类型的内部 API 演化为人体工学、类型安全的公共 API 的真实示例。考虑一个配置解析 crate：
...
pub fn parse_config(
    ...
    strictness: Strictness,  // 命名替代，而不是裸 bool
...
> **经验法则**：如果你发现自己编写对字符串值的 `match`，考虑用枚举替换参数。
> 如果参数是上下文中不明显的布尔值，使用双变体枚举代替。

***

### 解析不要验证 —— `TryFrom` 和已验证类型

"解析，不要验证"是一个原则，它说：**不要检查数据然后传递未经检查的原始形式 —— 而是解析为只有在数据有效时才能存在的类型。**
Rust 的 `TryFrom` trait 是此的标准工具。
...
#### 解决方案：使用 `TryFrom` 解析为已验证的新类型

```rust
use std::convert::TryFrom;
use std::fmt;

/// 已验证的 TCP 端口号（1–65535）。
/// 如果你有 `Port`，它保证有效。
...
对于通常从文本解析的类型（CLI 参数、配置文件），实现 `FromStr`：
...
| 已验证新类型 + `TryFrom` | ✅ | ✅ | 从不 —— 类型是证明 |

规则：**在边界处解析，在内部任何地方使用已验证类型。**
原始字符串、整数和字节切片进入你的系统，通过 `TryFrom`/`FromStr` 解析为已验证类型，从那时起类型系统保证它们是有效的。
...
- 使用 `dep:` 语法（Rust 1.60+）用于可选依赖以避免创建隐式特性
...
`.cargo/config.toml` 文件（在工作区根目录或 `$HOME/.cargo/` 中）
自定义 Cargo 行为而无需修改 `Cargo.toml`：
...
# Cargo 别名 —— 自定义快捷命令
[alias]
xt = "test --workspace --release"        # cargo xt = release 模式下运行所有测试
...
| 设置 | 目的 | 示例 |
|---|---|---|
| `[build] target` | 默认编译 target | `x86_64-unknown-linux-musl` 用于静态构建 |
...
| `[env]` | 构建时环境变量 | 库路径、特性切换 |
| `[net] offline` | 防止网络访问 | `true` 用于隔离构建 |

### 编译时环境变量：`env!()` 和 `option_env!()`

Rust 可以在编译时将环境变量嵌入二进制文件 —— 对版本字符串、构建元数据和配置很有用：

```rust
// env!() —— 如果变量缺失则在编译时 panic
const VERSION: &str = env!("CARGO_PKG_VERSION"); // Cargo.toml 中的 "0.1.0"
const PKG_NAME: &str = env!("CARGO_PKG_NAME");   // Cargo.toml 中的 crate 名称
...
`cfg_attr` **仅当**条件为真时应用属性。这比 `#[cfg()]` 更有针对性，后者包含/排除整个项：
...
| `#[cfg_attr(feature = "x", derive(Foo))]` | 仅当启用特性 "x" 时添加 `derive(Foo)` |
...
unmaintained = "warn"        # 警告不再维护的 crate
...
[sources]
allow-git = []              # 生产环境中无 git 依赖
```

| 工具 | 目的 | 何时运行 |
|---|---|---|
| `cargo audit` | 检查依赖中的已知 CVE | CI 流水线、发布前 |
| `cargo deny check` | 许可证、禁止、咨询、来源 | CI 流水线 |
...
> **完整覆盖**：参见 [使用 criterion 进行基准测试](ch14-testing-and-benchmarking-patterns.md#benchmarking-with-criterion)
> 第 13 章（测试和基准测试模式）中的完整 `criterion` 设置、API 示例和与 `cargo bench` 的比较表。
> 下面是特定于架构的用法的快速参考。

在对 crate 的公共 API 进行基准测试时，将基准测试放在 `benches/` 中，
并专注于热点路径 —— 通常是解析器、序列化器或验证边界：

```bash
cargo bench                  # 运行所有基准测试
cargo bench -- parse_config  # 运行特定基准测试
# 结果在 target/criterion/ 中，带有 HTML 报告
```

> **关键要点 —— 架构和 API 设计**
> - 接受最通用的类型（`impl Into`、`impl AsRef`、`Cow`）；返回最具体的
> - 解析不要验证：使用 `TryFrom` 创建通过构造有效的类型
> - 公共枚举上的 `#[non_exhaustive]` 防止添加变体时的破坏性变更
> - `#[must_use]` 捕获重要值的静默丢弃

> **另见：** [第 9 章 — 错误处理](ch10-error-handling-patterns.md) 了解公共 API 中的错误类型设计。[第 13 章 — 测试](ch14-testing-and-benchmarking-patterns.md) 了解测试 crate 的公共 API。

---

### 练习：Crate API 重构 ★★（约 30 分钟）

将以下"字符串类型"的 API 重构为使用 `TryFrom`、新类型和 builder 模式的 API：
...
设计一个 `ServerConfig`，带有已验证类型 `Host`、`Port`（1–65535）和 `MaxConnections`（1–10000），在解析时拒绝无效值。
