...
> **你将学到：** 在真实的硬件场景中动手练习正确构造模式——NVMe 管理命令、固件更新状态机、传感器流水线、PCIe phantom 类型、多协议健康检查和会话类型诊断协议。
>
...
- `GetLogPage` → `SmartLog`（温度、可用备用空间、读取的数据单元数）
- `GetFeature` → 特定功能的响应

要求：
1. 命令类型决定响应类型
2. 没有运行时调度——仅静态调度
3. 添加一个 `NamespaceId` newtype，防止将命名空间 ID 与其他 `u32` 混淆
...
impl NvmeAdminCmd for Identify {
    ...
    fn parse_response(&self, raw: &[u8]) -> io::Result<IdentifyResponse> {
        if raw.len() < 12 {
            return Err(io::Error::new(io::ErrorKind::InvalidData, "太短"));
        }
        Ok(IdentifyResponse {
...
stateDiagram-v2
    ...
    note right of Applying : 没有 abort() —— 不可逆
...
2. 上传只能从 Idle 开始
3. 验证需要上传完成
4. 应用只能在成功验证后发生——接受一个 `VerifiedImage` 证明令牌
5. 重启是应用后的唯一选项
6. 添加一个 `abort()` 方法，在 Uploading 和 Verifying 状态可用（但在 Applying 不可用——太晚了）

**提示：** 结合 type-state（ch05）与 capability 令牌（ch04）。
...
```rust,ignore
// --- 状态类型 ---
// 设计选择：这里我们将状态内联存储（`_state: S`），而不是使用
// `PhantomData<S>`（ch05 的方法）。这允许状态携带数据——
// 例如 `Uploading { bytes_sent: usize }` 跟踪进度。使用 `PhantomData`
// 当状态是纯标记（零大小）时；使用内联存储当
// 状态携带有意义的运行时数据时。
pub struct Idle;
pub struct Uploading { bytes_sent: usize }  // 非 ZST——携带进度数据
pub struct Verifying;
pub struct Applying;
pub struct Rebooting;
pub struct Complete;

/// 证明令牌：只能在 verify() 内部构造。
...
impl FwUpdate<Verifying> {
    /// 成功时，返回下一个状态和 VerifiedImage 证明令牌。
...
impl FwUpdate<Applying> {
    /// 消费 VerifiedImage 证明——不能在没有验证的情况下应用。
...
**关键点：**
- `abort()` 仅存在于 `FwUpdate<Uploading>` 和 `FwUpdate<Verifying>` 上——在 `FwUpdate<Applying>` 上调用它是**编译错误**，而不是运行时检查。
- `VerifiedImage` 有私有字段，所以只有 `verify()` 能创建它。
- `apply()` 消费证明令牌——你不能跳过验证。
...
编译器应该拒绝：将 `Celsius` 与 `Volts` 比较、将 `Watts` 与 `Rpm` 相加、在期望 `Volts` 的地方传递 `Millivolts`。
...
// --- 相同单位类型的算术 ---
// 注意：添加绝对温度（25°C + 30°C）在物理上
// 是有问题的——参见 ch06 关于 ΔT newtype 的讨论以获取更严格的方法。
// 这里为了练习保持简单。
...
// --- 泛型阈值检查器 ---
// 练习 3 扩展了 ch06 的 Threshold，带有泛型 ThresholdResult<T>
...
// let _: Millivolts = Volts(1.0);         // 错误：没有隐式强制转换
// let _ = Watts(100.0) + Rpm(3000);       // 错误：类型不匹配
```

**关键点：**
- 每个物理单位是一个不同的类型——不会意外混淆。
- `Mul<Amperes> for Volts` 生成 `Watts`，将 P = V × I 编码到类型系统中。
- 对相关单位（mV ↔ V、°C ↔ °F）的显式 `From` 转换。
- `Threshold<Celsius>` 只接受 `Celsius`——不能意外地阈值检查 RPM。

</details>

### 练习 4：PCIe Capability 遍历（Phantom 类型 + 验证边界）

为 PCIe capability 链表建模：

1. `RawCapability` —— 来自配置空间的未验证字节
2. `ValidCapability` —— 已解析和验证（通过 TryFrom）
3. 每种 capability 类型（MSI、MSI-X、PCIe Express、电源管理）有自己的
   phantom 类型寄存器布局
...
// --- Phantom 标记用于 capability 类型 ---
pub struct Msi;
pub struct MsiX;
pub struct PciExpress;
pub struct PowerMgmt;

// PCI capability ID 来自规范
...
impl TryFrom<RawCapability> for ValidCapability<PowerMgmt> {
    ...
    fn try_from(raw: RawCapability) -> Result<Self, Self::Error> {
        if raw.id != CAP_ID_PM { return Err("不是 PM capability"); }
        if raw.data.len() < 2 { return Err("PM 数据太短"); }
        Ok(ValidCapability {
...
impl TryFrom<RawCapability> for ValidCapability<Msi> {
    type Error = &'static str;
    fn try_from(raw: RawCapability) -> Result<Self, Self::Error> {
        if raw.id != CAP_ID_MSI { return Err("不是 MSI capability"); }
        if raw.data.len() < 6 { return Err("MSI 数据太短"); }
        Ok(ValidCapability {
            id: raw.id, next_ptr: raw.next_ptr,
            data: raw.data, _kind: PhantomData,
        })
...
impl<'a> CapabilityWalker<'a> {
    pub fn new(config_space: &'a [u8]) -> Self {
        // Capability 指针位于 PCI 配置空间偏移 0x34
...
- `pm_control()` 仅存在于 `ValidCapability<PowerMgmt>` 上——在 MSI capability 上调用它是编译错误。
- `CapabilityWalker` 迭代器生成原始 capability；调用者用 `TryFrom` 验证他们关心的那些。

</details>

### 练习 5：多协议健康检查（Capability 混合）

创建一个健康检查框架：

1. 定义原料 trait：`HasIpmi`、`HasRedfish`、`HasNvmeCli`、`HasGpio`
2. 创建混合 trait：
...
3. 构建一个 `FullPlatformController` 实现所有原料 trait
4. 构建一个 `StorageOnlyController` 只实现 `HasNvmeCli`
5. 验证 `StorageOnlyController` 获得 `StorageHealthMixin` 但不获得其他混合
...
**关键点：**
- Blanket `impl<T: HasIpmi + HasGpio> ThermalHealthMixin for T {}` —— 任何实现两个原料的类型自动获得混合。
- `StorageOnlyController` 只实现 `HasNvmeCli`，所以编译器授予它 `StorageHealthMixin` 但拒绝 `thermal_check()` 和 `bmc_health()` —— 不需要运行时检查。
- 添加新混合（例如 `NetworkHealthMixin: HasRedfish + HasGpio`）是一个 trait + 一个 blanket impl——现有控制器如果符合条件就自动获得它。
...
2. 转换到 `Running` 状态——发放 `N` 个执行令牌（每个测试用例一个）
3. 每个 `TestToken` 在测试运行时被消费——防止运行相同的测试两次
4. 在所有令牌被消费后，转换到 `Complete` 状态
...
/// 一次性测试令牌。不可 Clone，不可 Copy——在使用时被消费。
...
impl DiagSession<Running> {
    ...
    /// 转换到 Complete。
    ///
    /// **注意：** 此解决方案**不**强制所有令牌已被消费——`finish()` 可以在
    /// 仍有令牌剩余时调用。令牌将被简单地丢弃（它们不是 `#[must_use]`）。
...
// // running_session.report();      →  错误：没有方法 `report` 在 DiagSession<Running> 上
```

**关键点：**
- `TestToken` 不是 `Clone` 或 `Copy` —— 通过 `run_test(token)` 消费它时 move 它，所以重新运行相同的测试是编译错误。
- `report()` 仅存在于 `DiagSession<Complete>` 上——在运行中途调用它是不可能的。
- **高级**变体将使用 `DiagSession<Running, N>` 与 const 泛型，其中 `run_test` 返回 `DiagSession<Running, {N-1}>` 且 `finish` 仅在 `DiagSession<Running, 0>` 上可用——这确保在结束前*所有*令牌都被消费。

</details>

## 关键要点

1. **用真实协议练习** —— NVMe、固件更新、传感器流水线、PCIe 都是这些模式的真实世界目标。
2. **每个练习映射到核心章节** —— 使用交叉引用来在尝试之前复习模式。
3. **解决方案使用可展开的详情** —— 在揭示解决方案之前尝试每个练习。
4. **在练习 5 中组合模式** —— 多协议健康检查结合了类型化命令、量纲类型和验证边界。
5. **会话类型（练习 6）是前沿** —— 它们强制跨通道的消息排序，将 type-state 扩展到分布式系统。
