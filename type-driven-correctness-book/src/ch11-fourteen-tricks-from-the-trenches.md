...
> **你将学到：** 十四个较小的构造正确性技术——从哨兵消除和密封 trait 到 session 类型、`Pin`、RAII 和 `#[must_use]`——每个都以几乎零成本消除特定的 bug 类别。
>
> **交叉引用：** [ch02](ch02-typed-command-interfaces-request-determi.md)（密封 trait 扩展 ch02）、[ch05](ch05-protocol-state-machines-type-state-for-r.md)（typestate builder 扩展 ch05）、[ch07](ch07-validated-boundaries-parse-dont-validate.md)（FromStr 扩展 ch07）

## 十四个实战技巧

八个核心模式（ch02–ch09）涵盖了主要的构造正确性技术。本章收集了**十四个较小但高价值的技巧**，这些技巧在生产 Rust 代码中反复出现——每个都以零或几乎零成本消除特定类别的 bug。

### 技巧 1 —— 边界处的哨兵 → `Option`

硬件协议充满了哨兵值：IPMI 使用 `0xFF` 表示"传感器不存在"，PCI 使用 `0xFFFF` 表示"无设备"，SMBIOS 使用 `0x00` 表示"未知"。如果你将这些哨兵作为普通整数在代码中传递，每个消费者都必须记得检查这个魔术值。如果有一个比较忘记了，就会出现虚幻的 255°C 读数或虚假的 vendor-ID 匹配。

**规则：** 在最早的解析边界将哨兵转换为 `Option`，并且仅在序列化边界转换回哨兵。
...
current_vendor_id = u16::from_str_radix(hex, 16)
    .unwrap_or(0xFFFF);  // 哨兵隐藏了错误
```

每个接收 `current_vendor_id` 的函数都必须知道 `0xFFFF` 是特殊的。如果有人写 `if vendor_id == target_id` 而没有先检查 `0xFFFF`，当目标也恰好从错误数据解析为 `0xFFFF` 时，缺失的设备会静默匹配。
...
//     temp > threshold  // 错误：无法比较 Option<u8> 与 u8
// }
```
...
`pcie_tree/src/lspci.rs` 的重构很简单：将 `current_vendor_id: u16` 改为 `current_vendor_id: Option<u16>`，用 `None` 替换 `0xFFFF`，让编译器找到每个需要更新的站点。
...
第 2 章介绍了 `IpmiCmd`，它有一个关联类型将每个命令绑定到其响应。但有一个漏洞：如果*任何*代码都能实现 `IpmiCmd`，有人可以编写一个 `MaliciousCmd`，其 `parse_response` 返回错误的类型或 panic。整个系统的类型安全性取决于每个实现的正确性。

**密封 trait** 封闭了这个漏洞。想法很简单：让 trait 需要一个*私有的* 超 trait，只有你的 crate 可以实现。
...
在你的 crate 内部，你为每个批准的命令类型实现 `Sealed`：
...
外部代码看到 `IpmiCmd` 并可以调用 `execute()`，但无法实现它：

```rust,ignore
// 在另一个 crate 中：
struct EvilCmd;
// impl private::Sealed for EvilCmd {}  // 错误：模块 `private` 是私有的
// impl IpmiCmd for EvilCmd { ... }     // 错误：`Sealed` 不满足
```
...
| 密封当… | 不密封当… |
|---------|-----------|
| 安全性取决于正确的实现（IpmiCmd、DiagModule） | 用户应该扩展系统（自定义报告格式化程序） |
| 关联类型必须满足不变量 | trait 是简单的 capability 标记（HasIpmi） |
| 你拥有规范的实现集合 | 第三方插件是设计目标 |

#### 实际候选

- `IpmiCmd` —— 错误的解析可能污染类型化响应
- `DiagModule` —— 框架假设 `run()` 返回有效的 DER 记录
- `SelEventFilter` —— 损坏的过滤器可能吞掉关键的 SEL 事件

***

### 技巧 3 —— 用于演进枚举的 `#[non_exhaustive]`

`inventory/src/types.rs` 中的 `SkuVariant` 今天有五个变体：
...
当下一代出货且你添加 `S4001` 时，任何外部代码如果匹配 `SkuVariant` 且没有通配符臂，将**静默地编译失败**——这正是重点。但内部代码呢？没有 `#[non_exhaustive]`，你在*同一个 crate* 中的 `match` 编译时不需要通配符，添加新的变体会破坏你自己的构建。

标记枚举为 `#[non_exhaustive]` 强制**外部 crate** 在匹配时包含通配符臂。在定义的 crate 内，`#[non_exhaustive]` 没有效果——你仍然可以编写穷举匹配。

**为什么这有用：** 当你从库 crate（或 workspace 中的共享子 crate）发布 `SkuVariant` 时，下游代码被迫处理未知的未来变体。当你下一代添加 `S4001` 时，下游代码已经编译——他们有通配符臂。

```rust,ignore
// 在 gpu_sel crate（定义的 crate）中：
#[non_exhaustive]
...
```rust,ignore
// 在二进制 crate（依赖 inventory 的下游 crate）中：
fn diag_path_external(sku: inventory::SkuVariant) -> &'static str {
    match sku {
        ...
        _ => "generic_diag",  // #[non_exhaustive] 对外部 crate 要求
    }
}
```

> **Workspace 提示：** 如果你的所有代码都在单个 crate 中，`#[non_exhaustive]` 不会帮助——它只影响跨 crate 边界。对于项目的大型 workspace，将演进的枚举放在共享 crate（`core_lib` 或 `inventory`）中，这样属性可以保护其他 workspace crate 中的消费者。
...
| `SensorType` | `protocol_lib` | IPMI 规范保留 0xC0–0xFF 给 OEM |
...
第 5 章展示了用于*协议*的 type-state（会话生命周期、链路训练）。同样的想法适用于*builder* ——  structs 的 `build()` / `finish()` 只有在设置了所有必需字段后才能调用。
...
```rust,ignore
// 当前的 fluent builder —— finish() 总是可用
...
impl DerBuilder {
    ...
    pub fn finish(self) -> Der { self.der }  // ← 总是可调用！
}
```

这会编译没有错误，但产生不完整的 DER 记录：
...
#### Typestate builder：`finish()` 需要两个字段
...
// 设置 mnemonic（无论 fault_class 的状态如何都有效）
...
// 设置 fault_class（无论 mnemonic 的状态如何都有效）
...
let bad = DerBuilder::new("CSI_ERR", 62691)
    .mnemonic("ACCEL_CARD_ER691")
    .finish();  // 错误：方法 `finish` 未找到
```

#### 何时使用 typestate builder

| 使用当… | 不必麻烦当… |
|---------|-------------|
| 省略字段导致静默 bug（DER 缺少 mnemonic） | 所有字段都有 sensible 的默认值 |
| builder 是公共 API 的一部分 | builder 仅是测试脚手架 |
| 超过 2-3 个必需字段 | 单个必需字段（在 `new()` 中获取即可） |

***

### 技巧 5 —— `FromStr` 作为验证边界

第 7 章展示了 `TryFrom<&[u8]>` 用于二进制数据（FRU 记录、SEL 条目）。对于**字符串**输入——配置文件、CLI 参数、JSON 字段——类似的边界是 `FromStr`。

#### 问题

```rust,ignore
// C++ / 未验证的 Rust：静默回退到默认值
fn route_diag(level: &str) -> DiagMode {
    if level == "quick" { ... }
    else if level == "standard" { ... }
    else { QuickMode }  // 配置文件中的拼写错误？¯\_(ツ)_/¯
}
```

配置文件中有 `"diag_level": "extendedd"`（拼写错误）静默地获得 `QuickMode`。
...
1. **快速失败：** 错误输入在解析边界被捕获，而不是在诊断逻辑深处三层。
2. **别名是显式的：** `"MEM"`、`"DIMM"` 和 `"MEMORY"` 都映射到 `Component::Memory` —— match 臂记录了映射。
3. **`.parse()` 符合人体工程学：** 因为 `FromStr` 与 `str::parse()` 集成，你获得简洁的单行代码：`let level: DiagLevel = config["level"].parse()?;`

#### 实际代码库使用

项目已经有 8 个 `FromStr` 实现：
...
| `SkuVariant` | `inventory` | 相同的别名（单独的模块，相同的模式） |
| `FaultStatus` | `config_loader` | Fault 生命周期状态 |
...
| 章节 | ch07 | ch11 |
| 两者都使用 | `Result` —— 强制调用者处理无效输入 |

***

### 技巧 6 —— 用于编译时大小验证的 Const 泛型

当硬件缓冲区、寄存器组或协议帧有固定大小时，const 泛型让编译器强制执行它们：
...
// NvmeBuffer::<1024>::new();  // 以这种形式在运行时 panic
// 对于真正的编译时强制执行，参见技巧 9（const 断言）。
```

> **何时使用：** 固定大小的协议缓冲区（NVMe、PCIe 配置空间）、DMA 描述符、硬件 FIFO 深度。任何大小是硬件常量的地方，永远不应该在运行时变化。

***

### 技巧 7 —— `unsafe` 周围的安全包装器

项目目前有零个 `unsafe` 块。但当你添加 MMIO 寄存器访问、DMA 或与 accel-mgmt/accel-query 的 FFI 时，你将需要 `unsafe`。构造正确性方法：**将每个 `unsafe` 块包装在安全抽象中**，这样不安全性被包含且可审计。

```rust,ignore
/// MMIO 映射的寄存器。指针对映射的生命周期有效。
...
impl MmioRegion {
    ...
    pub fn read_u32(&self, offset: usize) -> Option<u32> {
        ...
        // SAFETY: offset 在上面进行了边界检查，base 根据 new() 契约有效
        Some(unsafe {
...
**与 phantom 类型（ch09）结合用于类型化 MMIO：**
...
impl TypedMmio<ReadOnly> {
    ...
    // 没有 write 方法——如果你尝试写入 ReadOnly 区域是编译错误
}
...
> | 规则 | 为什么 |
> |------|--------|
> | 一个带有文档化 `# Safety` 不变量的 `unsafe fn new()` | 调用者一次性承担责任 |
> | 所有其他方法是安全的 | 调用者无法触发 UB |
> | 每个 `unsafe` 块上的 `# SAFETY:` 注释 | 审计者可以在本地验证 |
> | 用 `#[deny(unsafe_op_in_unsafe_fn)]` 包装在模块中 | 即使在 `unsafe fn` 内部，单个操作也需要 `unsafe` |
...
你现在有七个日常技巧。这是一个快速计分卡：

| 技巧 | 消除的 bug 类别 | 采用成本 |
|:----:|----------------|:--------:|
| 1 | 哨兵混淆（0xFF） | 低——边界处一个 `match` |
| 2 | 未授权的 trait 实现 | 低——添加 `Sealed` 超 trait |
| 3 | 枚举增长后的破坏的消费者 | 低——一行属性 |
| 4 | 缺失 builder 字段 | 中——额外的类型参数 |
| 5 | 字符串类型配置中的拼写错误 | 低——`impl FromStr` |
| 6 | 错误的缓冲区大小 | 低——const 泛型参数 |
| 7 | unsafe 散布在代码库中 | 中——包装器模块 |

技巧 8-14 **更高级**——它们涉及 async、const 求值、session 类型、`Pin` 和 `Drop`。如果需要可以在这里休息一下；上面的技术已经是高价值、低成本的胜利，你明天就可以采用。

***

### 技巧 8 —— Async Type-State 机

当硬件驱动使用 `async`（例如异步 BMC 通信、异步 NVMe I/O）时，type-state 仍然有效——但在 `.await` 点之间的所有权需要注意：
...
impl AsyncSession<Idle> {
    ...
    /// 过渡 Idle → Authenticating → Active。
    /// Session 被消费（移动进入 future）跨越 .await。
    pub async fn authenticate(self, user: &str, pass: &str)
        -> Result<AsyncSession<Active>, String>
    {
        // 阶段 1：发送凭证（消费 Idle session）
...
**async type-state 的关键规则：**

| 规则 | 为什么 |
|------|--------|
| 过渡方法获取 `self`（按值），而不是 `&mut self` | 所有权转移在 `.await` 之间有效 |
| 返回 `Result<NextState, (Error, PrevState)>` 用于可恢复错误 | 调用者可以从 previous 状态重试 |
| 不要将状态分割到多个 futures 中 | 一个 future 拥有一个 session |
| 如果使用 tokio::spawn 使用 `Send + 'static` 边界 | session 必须可以跨线程移动 |

> **注意：** 如果你需要在错误时取回*previous*状态（用于重试），返回 `Result<AsyncSession<Active>, (Error, AsyncSession<Idle>)>` 以便调用者取回所有权。没有这个，失败的 `.await` 会永久丢弃 session。

***

### 技巧 9 —— 通过 Const 断言的精炼类型

当数值约束是编译时不变量（不是运行时数据）时，使用 `const` 求值强制执行它。这与技巧 6（提供类型级大小区分）不同——这里我们在编译时*拒绝无效值*：

```rust,ignore
/// 必须在 IPMI SDR 范围内的传感器 ID（0x01..=0xFE）。
...
impl<const N: u8> SdrSensorId<N> {
    ...
    pub const fn validate() {
        ...
        assert!(N <= 0xFE, "传感器 ID 必须 <= 0xFE（0xFF 保留）");
    }
...
// read_sensor_const::<0x20>();   // ✅ 编译通过——0x20 有效
// read_sensor_const::<0x00>();   // ❌ 编译错误——"传感器 ID 必须 >= 0x01"
// read_sensor_const::<0xFF>();   // ❌ 编译错误——0xFF 保留
```
...
impl<const N: u8> BoundedFanId<N> {
    pub const VALIDATED: () = assert!(N < 8, "服务器最多 8 个风扇（0..7）");
...
> **何时使用：** 编译时已知的硬件定义的固定 ID（传感器 ID、风扇插槽、PCIe 插槽号）。当值来自运行时数据（配置文件、用户输入）时，改用 `TryFrom` / `FromStr`（ch07、技巧 5）。

***

### 技巧 10 —— 用于通道通信的 Session 类型

当两个组件通过通道通信时（例如诊断编排器 ↔ 工作线程），**session 类型**在类型系统中编码协议：
...
/// 类型化的通道端点。`S` 是当前协议状态。
pub struct Chan<S> {
    // 在实际代码中：包装 mpsc::Sender/Receiver 对
    _state: PhantomData<S>,
}

impl Chan<SendRequest> {
    /// 发送请求——过渡到 RecvResponse 状态。
...
fn orchestrator(chan: Chan<SendRequest>) {
    ...
    chan.close();
    println!("结果：{}", if response.passed { "PASS" } else { "FAIL" });
}

// 不能在发送前接收：
// fn wrong_order(chan: Chan<SendRequest>) {
//     chan.recv();  // ❌ 在 Chan<SendRequest> 上没有方法 `recv`
// }
```
...
一些 type-state 机需要持有对自身数据的引用（例如其拥有的缓冲区内跟踪位置的分析器）。Rust 通常禁止这一点，因为移动结构体会使内部指针无效。`Pin<T>` 通过保证值**不会被移动**来解决这个问题：
...
pub struct StreamParser {
    ...
    /// 指向 `buffer`。仅在固定时有效。
    cursor: *const u8,
    _pin: PhantomPinned,  // 退出 Unpin——防止意外解固定
}
...
**关键洞察：** `Pin` 是解决自引用结构体问题的构造正确性方案。没有它，你将需要 `unsafe` 和手动生命周期跟踪。有了它，编译器防止移动且内部指针不变量得到维护。

| 使用 `Pin` 当… | 不使用 `Pin` 当… |
|----------------|-----------------|
| 状态机持有结构体内引用 | 所有字段独立拥有 |
| 异步 futures 在 `.await` 之间借用 | 不需要自引用 |
| DMA 描述符在内存中不能重定位 | 数据可以自由移动 |
| 带有内部 cursor 的硬件环形缓冲区 | 简单的基于索引的迭代有效 |

***

### 技巧 12 —— RAII / `Drop` 作为正确性保证

Rust 的 `Drop` trait 是一个构造正确性机制：清理代码**不可能被忘记**，因为编译器自动插入它。这对于必须恰好一次释放的硬件资源特别有价值。

```rust,ignore
use std::io;

/// 完成后*必须*关闭的 IPMI 会话。
/// `Drop` impl 保证清理即使在 panic 或提前 `?` 返回时。
...
impl Drop for IpmiSession {
    fn drop(&mut self) {
        // Close Session 命令：总是运行，即使在 panic/提前返回时。
        // 在 C 中，忘记 CloseSession() 会泄漏 BMC 会话插槽。
...
**将 RAII 与 type-state（ch05）结合用于有序清理：**

你不能为泛型参数特化 `Drop`（Rust 错误 E0366）。相反，对每个状态使用**单独的包装类型**：
...
impl Drop for LockedGpu {
    fn drop(&mut self) {
        // drop 时解锁时钟——仅针对锁定的包装器触发。
        eprintln!("[RAII] GPU {} 时钟已解锁", self.device_id);
    }
}

// GpuContext<Open> 没有特殊的 Drop——没有时钟需要解锁。
// LockedGpu 总是在 drop 时解锁，即使在 panic 或提前返回时。
```

> **为什么不 `impl Drop for GpuContext<Locked>`？** Rust 要求 `Drop` impl 应用于泛型类型的*所有*实例。要获得特定状态的清理，使用以下之一：
>
> | 方法 | 优点 | 缺点 |
> |------|------|------|
> | 单独的包装类型（如上） | 干净、零成本 | 额外的类型名 |
> | 泛型 `Drop` + 运行时 `TypeId` 检查 | 单一类型 | 需要 `'static`，运行时成本 |
> | `Drop` 中的 `enum` 状态与穷举匹配 | 单一泛型类型 | 运行时分发，较少的类型安全性 |

> **何时使用：** BMC 会话、GPU 时钟锁定、DMA 缓冲区映射、文件句柄、互斥守卫、任何带有强制释放步骤的资源。如果你发现自己写 `fn close(&mut self)` 或 `fn cleanup()`，它几乎肯定应该是 `Drop`。

***

### 技巧 13 —— 作为正确性的错误类型层次结构

精心设计的错误类型防止静默错误吞没，确保调用者适当地处理每种失败模式。使用 `thiserror` 进行结构化错误处理是一个构造正确性模式：编译器强制穷举匹配。
...
| 调用者获得不透明的字符串 | 调用者匹配特定的变体 |
| 无法区分认证失败与超时 | `DiagError::Ipmi(IpmiError::AuthFailed)` vs `Timeout` |
| 日志记录吞掉错误 | `match` 强制处理每个情况 |
| 新错误变体 → 没有人注意 | 新变体 → 编译器警告未匹配的臂 |
...
|---------------------|------------------|
| 编写库/crate | 编写二进制/CLI |
| 调用者需要匹配错误变体 | 调用者只是日志记录并退出 |
| 错误类型是公共 API 的一部分 | 内部错误管道 |
| `protocol_lib`, `accel_diag`, `thermal_diag` | `diag_tool` 主二进制 |

> **何时使用：** workspace 中的每个 crate 都应该用 `thiserror` 定义它自己的错误枚举。顶层二进制 crate 可以使用 `anyhow` 来聚合它们。这给库调用者编译时错误处理保证，同时保持二者的简洁性。

***

### 技巧 14 —— 用于强制执行消费的 `#[must_use]`

`#[must_use]` 属性将忽略的返回值转换为编译器警告。这是一个轻量级的构造正确性工具，可以与本指南中的每个模式配对：

```rust,ignore
/// 校准令牌，*必须*使用——静默丢弃它是一个 bug。
#[must_use = "校准令牌必须传递给 calibrate()，不能丢弃"]
pub struct CalibrationToken {
    _private: (),
}

/// 诊断结果，*必须*检查——忽略失败是一个 bug。
#[must_use = "诊断结果必须检查失败"]
...
```text
警告：未使用必须使用的 `CalibrationToken`
  --> src/main.rs:5:5
   |
5  |     CalibrationToken { _private: () };
   |     ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
   |
   = 注意：校准令牌必须传递给 calibrate()，不能丢弃
```
...
| 模式 | 注释什么 | 为什么 |
|------|---------|--------|
| 一次性令牌（ch03） | `CalibrationToken`, `FusePayload` | 不使用就丢弃 = 逻辑 bug |
| Capability 令牌（ch04） | `AdminToken` | 认证但忽略令牌 |
| Type-State 过渡 | `authenticate()`, `activate()` 的返回类型 | 创建的会话从未使用 |
| 结果 | `DiagResult`, `SensorReading` | 静默失败吞没 |
| RAII 句柄（技巧 12） | `IpmiSession`, `LockedGpu` | 打开但不使用资源 |

> **经验法则：** 如果丢弃一个值而不使用它总是一个 bug，添加 `#[must_use]`。如果有时是故意的（例如 `Vec`），不要。`_` 前缀（`let _ = foo()`）显式确认并消除警告——当丢弃是故意的时这没问题。

## 关键要点

1. **边界处的哨兵 → Option** ——在解析时将魔术值转换为 `Option`；编译器强制调用者处理 `None`。
2. **密封 trait 封闭实现漏洞** ——私有超 trait 意味着只有你的 crate 可以实现 trait。
3. **`#[non_exhaustive]` + `#[must_use]` 是一行、高价值的注释** ——将它们添加到演进的枚举和消费的令牌。
4. **Typestate builder 强制执行必需字段** ——当所有必需的参数是 `Set` 时，`finish()` 才存在。
5. **每个技巧针对特定的 bug 类别** ——增量采用它们；没有技巧需要重写你的架构。
