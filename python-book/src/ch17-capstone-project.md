...
> **你将学到：** 通过构建一个完整的 Rust CLI 应用来综合运用课程中的所有知识，
> 这种应用 Python 开发者通常会用 `argparse` + `json` + `pathlib` 来编写。
>
> **难度：** 🔴 高级

这个综合项目练习了来自每个主要章节的概念：
- **第 3 章**：类型和变量（struct、enum）
- **第 5 章**：集合（`Vec`、`HashMap`）
- **第 6 章**：enum 和模式匹配（任务状态、命令）
- **第 7 章**：所有权和借用（传递引用）
...
> **Python 对比**：在 Python 中你会使用 `@dataclass` + `Enum`。在 Rust 中，`struct` + `enum` + `derive` 宏免费给你序列化、Display 和解析功能。
...
> **Python 对比**：Python 使用 `argparse` 或 `click`。这个手写的解析器展示了如何在 enum 上使用 `match` 来替代 Python 的 if/elif 链。对于真实项目，使用 `clap` crate。
...
> **使用的关键 Rust 模式**：`iter().map().max()`、`iter().filter().collect()`、`iter_mut().find()`、`retain()`、`iter().filter().count()`。这些替代了 Python 的列表推导式、`next(x for x in ...)` 和 `Counter` 模式。
...
> **Python 等价**：这相当于 `pyproject.toml` 的 `[project.dependencies]`。`cargo add serde serde_json chrono dirs` 类似于 `pip install`。
...
> **Python 等价**：`pytest` 测试。使用 `cargo test` 而不是 `pytest`。不需要测试发现机制 —— `#[test]` 显式标记测试函数。
...
3. **添加截止日期** —— 添加 `Option<NaiveDate>` 字段并过滤逾期任务。

4. **添加标签/分类** —— 使用 `Vec<String>` 存储标签并使用 `.iter().any()` 过滤。

5. **做成库 + 二进制** —— 拆分为 `lib.rs` + `main.rs` 以便逻辑可重用（第 8 章模块模式）。
...
| 章节 | 概念 | 出现位置 |
|------|------|----------|
| 第 3 章 | 类型和变量 | `Task` struct 字段、`u32`、`String`、`bool` |
| 第 5 章 | 集合 | `Vec<Task>`、`retain()`、`push()` |
| 第 6 章 | Enum + match | `Priority`、`Command`、穷尽匹配 |
| 第 7 章 | 所有权 + 借用 | `&[Task]` 与 `Vec<Task>`、`&mut` 用于完成 |
...
> 🎓 **恭喜！** 如果你构建了这个项目，你已经使用了本书涵盖的所有主要 Rust 概念。你不再是一个学习 Rust 的 Python 开发者 —— 你是一个懂 Rust 的开发者，同时也知道 Python。
