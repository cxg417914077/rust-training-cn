...
> **你将学到：** 多行闭包（不只是单表达式 lambda）、`Fn`/`FnMut`/`FnOnce` 捕获语义、
> 迭代器链 vs 列表推导式、`map`/`filter`/`fold`，以及 `macro_rules!` 宏基础。
>
> **难度：** 🟡 中级
...
```python
# Python —— 闭包通过引用捕获（延迟绑定陷阱！）
funcs = [lambda: i for i in range(3)]
print([f() for f in funcs])  # [2, 2, 2] —— 没想到吧！都捕获了同一个 `i`
...
本节将 Python 的推导式语法与 Rust 的迭代器链进行对比。

### 列表推导 → map/filter/collect
...
> **核心洞察**：Rust 迭代器是惰性的 —— 在调用 `.collect()` 之前什么都不会发生。Python 的生成器也是类似的，但列表推导是立即求值的。
...
| `any(f(x) for x in iter)` | `.any(f)` | 任一匹配则为 true |
| `all(f(x) for x in iter)` | `.all(f)` | 全部匹配则为 true |
| `enumerate(iter)` | `.enumerate()` | 索引 + 值 |
| `zip(a, b)` | `a.zip(b)` | 配对元素 |
| `len(list)` | `.count()`（消耗迭代器！）或 `.len()` | 计数元素 |
...
| `list(iter)` | `.collect::<Vec<_>>()` | 具体化为集合 |
...
Python 没有宏系统 —— 它使用装饰器、元类和运行时内省进行元编程。
Rust 使用宏在编译时生成代码。
...
@dataclass              # 在导入时生成 `__init__`、`__repr__`、`__eq__` 方法
class Point:
    x: float
    y: float
...
println!("Hello, {}!", name);           // 带格式化打印
format!("Value: {}", x);               // 创建格式化 String
vec![1, 2, 3];                          // 创建 Vec
assert_eq!(2 + 2, 4);                  // 测试断言
assert!(value > 0, "must be positive"); // 布尔断言
dbg!(expression);                       // 调试打印：打印表达式和值
todo!();                                // 占位符 —— 可编译但如果到达则 panic
unimplemented!();                       // 标记代码为未实现
panic!("something went wrong");         // 崩溃并带消息（类似 raise RuntimeError）
...
```rust
// `#[derive(...)]` 是 Python `@dataclass` 装饰器的 Rust 等价物
...
// Hash          → 可用作 HashMap 键（类似 __hash__）
// Default       → MyType::default()（类似无参 __init__）
...
| `@total_ordering` | `#[derive(PartialOrd, Ord)]` | 完整排序 |
| JSON `json.dumps(obj.__dict__)` | `#[derive(Serialize)]` | 序列化 |
| `MyClass(**json.loads(s))` | `#[derive(Deserialize)]` | 反序列化 |
...
**挑战**：创建一个 `User` 结构体，字段包括 `name: String`、`email: String` 和 `password_hash: String`。
派生 `Clone` 和 `PartialEq`，但手动实现 `Debug`，使其打印姓名和邮箱，但隐藏密码（显示 `"***"`）。
...
**核心要点**：与 Python 的 `__repr__` 不同，Rust 可以免费派生 `Debug` —— 但你可以为敏感字段覆盖它。
这比 Python 更安全，因为在 Python 中 `print(user)` 可能意外泄露敏感信息。
