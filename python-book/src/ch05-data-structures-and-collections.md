...
> **你将学到：** Rust 元组与 Python 元组的对比、数组和切片、结构体（Rust 替代类的方案）、
> `Vec<T>` 与 `list`、`HashMap<K,V>` 与 `dict`，以及用于领域建模的新类型模式。
>
> **难度：** 🟢 初学者
...
// - 需要命名字段      → 使用结构体
// - 需要方法          → 使用结构体
// （与 Python 相同的指导：tuple vs namedtuple vs dataclass）
```
...
```python
# Python — 列表是动态的、可包含异质类型
numbers = [1, 2, 3, 4, 5]       # 可以增长、缩小、包含混合类型
numbers.append(6)
mixed = [1, "two", 3.0]         # 允许混合类型
```

```rust
// Rust 有"两种"类型：固定大小和动态大小

// 1. 数组 — 固定大小，栈上分配（没有 Python 等价物）
...
// 2. 切片 — 数组或 Vec 的视图（类似 Python 切片，但是借用的）
let slice: &[i32] = &numbers[1..4]; // [2, 3, 4] — 是引用，不是拷贝！

// Python: numbers[1:4] 创建新列表（拷贝）
// Rust:   &numbers[1..4] 创建视图（无拷贝，无分配）
```
...
impl Rectangle {
    ...
    fn new(width: f64, height: f64) -> Self {
        Rectangle { width, height }   // 字段名相同时的简写
    }
...
flowchart LR
    subgraph Python ["Python 对象（堆）"]
        PH["PyObject 头<br/>（refcount + type ptr）"] --> PW["width: float obj"]
        PH --> PHT["height: float obj"]
        PH --> PD["__dict__"]
    end
    subgraph Rust ["Rust 结构体（栈）"]
        RW["width: f64<br/>（8 字节）"] --- RH["height: f64<br/>（8 字节）"]
    end
    style Python fill:#ffeeba
    style Rust fill:#d4edda
```

> **内存洞察**：Python `Rectangle` 对象有 56 字节的头部 + 独立的堆分配浮点对象。Rust `Rectangle` 正好是栈上的 16 字节 — 无间接、无 GC 压力。
>
> 📌 **另见**：[第 10 章 — 特征与泛型](ch10-traits-and-generics.md) 涵盖为你的结构体实现 `Display`、`Debug` 等特征以及运算符重载。

### 关键映射：Python 魔术方法 → Rust 特征
...
| `__len__` | `.len()` 方法 | 长度 |
| `__enter__`/`__exit__` | RAII + `impl Drop` | 自动清理；没有直接对应 Python 上下文管理器两阶段协议的等价物 |
...
> 两者效果相似，但 Rust 避免了菱形问题和脆弱基类问题。

***

## Vec vs list

`Vec<T>` 是 Rust 的可增长、堆分配数组 — 最接近 Python `list` 的等价物。
...
let empty: Vec<i32> = Vec::new();        // 空向量（需要类型注解）
...
let index = nums.iter().position(|&x| x == 3); // 返回 Some(索引) 或 None
```
...
| `lst.remove(x)` | `vec.iter().position(\|v\| v == &x).map(\|i\| vec.remove(i))` | 仅移除第一个匹配项（用 `retain` 移除所有） |
| `del lst[i]` | `vec.remove(i)` | 返回移除的元素 |
| `len(lst)` | `vec.len()` | |
| `x in lst` | `vec.contains(&x)` | |
| `lst.sort()` | `vec.sort()` | |
| `sorted(lst)` | 克隆 + 排序，或迭代器 | |
| `lst[i]` | `vec[i]` | 越界则 panic |
...
`HashMap<K, V>` 是 Rust 的哈希表 — 等同于 Python 的 `dict`。
...
// entry API = 对应 Python 的 setdefault / defaultdict 模式
...
**挑战**：编写一个函数，接受 `&str` 句子，返回 `HashMap<String, usize>` 的词频（不区分大小写）。
Python 中是 `Counter(s.lower().split())`。把它翻译成 Rust。
...
**核心要点**：`HashMap::entry().or_insert()` 是 Python `defaultdict` 或 `Counter` 的 Rust 等价物。
需要 `*` 解引用，因为 `or_insert` 返回 `&mut usize`。
