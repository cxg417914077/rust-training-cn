...
cargo build -vv                      # 详细模式——显示 build.rs 输出
...
cargo flamegraph -- --args           # 从二进制文件生成火焰图
...
sccache --show-stats                 # 缓存命中率统计
cargo nextest run                    # 更快的测试运行器
cargo nextest run --retries 2        # 重试不稳定的测试

# ─── 平台工程 ───
cargo check --target thumbv7em-none-eabihf   # 验证 no_std 构建
cargo build --target x86_64-pc-windows-gnu   # 跨平台编译到 Windows
cargo xwin build --target x86_64-pc-windows-msvc  # MSVC ABI 跨平台编译
cfg!(target_os = "linux")                    # 编译时 cfg（计算为 bool）

# ─── 发布 ───
cargo release patch --dry-run        # 预览发布
cargo release patch --execute        # 升级版本、提交、打标签、发布
...
| 缓冲区溢出检测 | ASan | `unsafe` 指针运算 |
| 泄漏检测 | Valgrind 或 LSan | 长时间运行的服务 |
| 本地 CI 等价物 | `cargo-make` | 开发者工作流自动化 |
| 提交前检查 | `cargo-husky` 或 git hooks | 推送前捕获问题 |
| 自动化发布 | `cargo-release` + `cargo-dist` | 版本管理 + 分发 |
| 依赖审计 | `cargo-audit` / `cargo-deny` | 供应链安全 |
| 许可证合规 | `cargo-deny` (licenses) | 商业/企业项目 |
| 供应链信任 | `cargo-vet` | 高安全环境 |
| 查找过时依赖 | `cargo-outdated` | 定期维护 |
| 检测破坏性变更 | `cargo-semver-checks` | 库 crate 发布 |
| 依赖树分析 | `cargo tree --duplicates` | 去重和精简依赖图 |
| 二进制大小分析 | `cargo-bloat` | 大小受限的部署 |
| 查找未使用依赖 | `cargo-udeps` / `cargo-machete` | 精简编译时间和大小 |
| LTO 调优 | `lto = true` 或 `"thin"` | Release 二进制优化 |
| 大小优化二进制 | `opt-level = "z"` + `strip = true` | 嵌入式 / WASM / 容器 |
| Unsafe 使用审计 | `cargo-geiger` | 安全策略执行 |
| 宏调试 | `cargo-expand` | Derive / macro_rules 调试 |
| 更快链接 | `mold` 链接器 | 开发者内部循环 |
| 编译缓存 | `sccache` | CI 和本地构建速度 |
| 更快测试 | `cargo-nextest` | CI 和本地测试速度 |
| MSRV 合规 | `cargo-msrv` | 库发布 |
| `no_std` 库 | `#![no_std]` + `default-features = false` | 嵌入式，UEFI, WASM |
| Windows 跨平台编译 | `cargo-xwin` / MinGW | Linux → Windows 构建 |
| 平台抽象 | `#[cfg]` + trait 模式 | 多操作系统代码库 |
| Windows API 调用 | `windows-sys` / `windows` crate | 原生 Windows 功能 |
| 端到端计时 | `hyperfine` | 完整二进制基准测试、前后对比 |
| 基于属性的测试 | `proptest` | 边界情况发现、解析器鲁棒性 |
| 快照测试 | `insta` | 大型结构化输出验证 |
| 覆盖率引导模糊测试 | `cargo-fuzz` | 解析器崩溃发现 |
| 并发模型检查 | `loom` | 无锁数据结构、原子序 |
| Feature 组合测试 | `cargo-hack` | 具有多个 `#[cfg]` feature 的 crate |
| 快速 UB 检查（近原生） | `cargo-careful` | CI 安全门控，比 Miri 更轻量 |
| 保存时自动重建 | `cargo-watch` | 开发者内部循环、紧密反馈 |
| Workspace 文档 | `cargo doc` + rustdoc | API 发现、入职、文档链接 CI |
| 可重现构建 | `--locked` + `SOURCE_DATE_EPOCH` | 发布完整性验证 |
| CI 缓存调优 | `Swatinem/rust-cache@v2` | 构建时间减少（冷 → 缓存） |
| Workspace lint 策略 | `[workspace.lints]` 在 Cargo.toml 中 | 所有 crate 的一致 Clippy/编译器 lint |
| 自动修复 lint 警告 | `cargo clippy --fix` | 自动化清理琐碎问题 |
...
| 跨平台编译 | [Rust Cross-Compilation](https://rust-lang.github.io/rustup/cross-compilation.html) |
...
| 性能引导优化 | [Rust PGO guide](https://doc.rust-lang.org/rustc/profile-guided-optimization.html) |
...
*作为 Rust Patterns 和 Type-Driven Correctness 的配套参考生成*

*Version 1.3 — 新增 cargo-hack, cargo-careful, cargo-watch, cargo doc, 可重现构建，CI 缓存策略，capstone 练习，以及完整的章节依赖图。*
